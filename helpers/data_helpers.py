...
from datasets.pointda import PointDA, PointDA_DOMAINS, PointDA_CLASSES
...
def get_dataset(dataset_name, root, transform, val_transform=None):
    ...
    # 这里加入对pointda的支持
    elif dataset_name == "shapenet-modelnet":
        trainset = PointDA(root, domains='shapenet', train=True, transform=transform)
        valset = PointDA(root, domains='shapenet', train=False, transform=transform)
        testset = PointDA(root, domains='modelnet',train=False, transform=transform)
    elif dataset_name == "modelnet-shapenet":
        trainset = PointDA(root, domains='modelnet', train=True, transform=transform)
        valset = PointDA(root, domains='modelnet', train=False, transform=transform)
        testset = PointDA(root, domains='shapenet',train=False, transform=transform)
    else:
